...
import com.example.practicahibernate1n_eduardodominguez.Util.MostrarEscena;
...
public class CocheController implements Initializable {
    ...
    void onActualizarClic(ActionEvent event) {
        ...
        if (CocheDAO.getInstance().actualizarCoche(cocheSeleccionado)) {
    ...
    void onBorrarClic(ActionEvent event) {
        ...
        if (CocheDAO.getInstance().borrarCoche(coche)) {
    ...
    void onInsertarClic(ActionEvent event) {
        ...
        if (CocheDAO.getInstance().verificarExisteMatricula(matricula)) {
        ...
        if (CocheDAO.getInstance().insertarCoche(coche1)) {
    ...
    void actualizarTabla() {
        ...
        List<Coche> listadoCoches = CocheDAO.getInstance().obtenerCoche(); // Obtengo la lista de coches desde la base de datos
    ...
    public void onVerMultasClic(ActionEvent event) throws IOException {
        MostrarEscena.mostrarScene(event,"/ui/multas.fxml");
    }
}